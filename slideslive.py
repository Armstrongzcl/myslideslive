...
def get_sl_info(sl_id):
    ...
    #print(info_request.content.decode())
    info_json = {}
    #json.loads(info_request.content.decode())
    req_content = info_request.content.decode().split("\n")
    for item in req_content:
        if "SLIDES-JSON-URL" in item:
            item_slice = item.split(":")
            info_json["slides_json_url"] = item_slice[1] + ":" + item_slice[2]
    #print(info_json)
    return info_json
